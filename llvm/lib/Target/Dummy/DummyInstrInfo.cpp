//===-- DummyInstrInfo.cpp - Dummy Instruction Information ----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Dummy implementation of the TargetInstrInfo class.
//
//===----------------------------------------------------------------------===//

#include "DummyInstrInfo.h"
#include "Dummy.h"
#include "DummyMachineFunctionInfo.h"
...
void DummyInstrInfo::anchor() {}

DummyInstrInfo::DummyInstrInfo(DummySubtarget &ST)
    : DummyGenInstrInfo(SP::ADJCALLSTACKDOWN, SP::ADJCALLSTACKUP), RI(),
      Subtarget(ST) {}
...
unsigned DummyInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
...
unsigned DummyInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                            int &FrameIndex) const {
...
bool DummyInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
...
unsigned DummyInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                      MachineBasicBlock *TBB,
                                      MachineBasicBlock *FBB,
  ...
  assert((Cond.size() == 1 || Cond.size() == 0) &&
         "Dummy branch conditions should have one component!");
...
unsigned DummyInstrInfo::removeBranch(MachineBasicBlock &MBB,
...
bool DummyInstrInfo::reverseBranchCondition(
...
void DummyInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
...
Register DummyInstrInfo::getGlobalBaseReg(MachineFunction *MF) const {
  DummyMachineFunctionInfo *DummyFI = MF->getInfo<DummyMachineFunctionInfo>();
  Register GlobalBaseReg = DummyFI->getGlobalBaseReg();
  if (GlobalBaseReg)
  ...
  DummyFI->setGlobalBaseReg(GlobalBaseReg);
  return GlobalBaseReg;
}
