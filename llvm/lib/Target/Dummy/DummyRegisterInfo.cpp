//===-- DummyRegisterInfo.cpp - SPARC Register Information ----------------===//
//
...
DummyRegisterInfo::DummyRegisterInfo() : DummyGenRegisterInfo(SP::O7) {}

const MCPhysReg*
DummyRegisterInfo::getCalleeSavedRegs(const MachineFunction *MF) const {
  return CSR_SaveList;
}

const uint32_t *
DummyRegisterInfo::getCallPreservedMask(const MachineFunction &MF,
                                        CallingConv::ID CC) const {
  return CSR_RegMask;
}

const uint32_t*
DummyRegisterInfo::getRTCallPreservedMask(CallingConv::ID CC) const {
  return RTCSR_RegMask;
}

BitVector DummyRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  BitVector Reserved(getNumRegs());
  const DummySubtarget &Subtarget = MF.getSubtarget<DummySubtarget>();
...
DummyRegisterInfo::getPointerRegClass(const MachineFunction &MF,
                                      unsigned Kind) const {
  const DummySubtarget &Subtarget = MF.getSubtarget<DummySubtarget>();
...
void
DummyRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
...
// Dummy has no architectural need for stack realignment support,
// except that LLVM unfortunately currently implements overaligned
// stack objects by depending upon stack realignment support.
// If that ever changes, this can probably be deleted.
bool DummyRegisterInfo::canRealignStack(const MachineFunction &MF) const {
  if (!TargetRegisterInfo::canRealignStack(MF))
    return false;

  // Dummy always has a fixed frame pointer register, so don't need to
