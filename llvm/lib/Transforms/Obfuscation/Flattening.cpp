...
bool Flattening::runOnFunction(Function &F) {
  DEBUG_WITH_TYPE(DEBUG_TYPE, errs() << "fla: Function " << F.getName()
                    <<", before the pass:\n");
...
bool Flattening::flatten(Function *f) {
  ...
  unsigned scrambling_key_int = llvm::cryptoutils->scramble32(0, scrambling_key);
  new StoreInst(
  ...
  for (vector<BasicBlock *>::iterator b = origBB.begin(); b != origBB.end();
    ...
    // Add case to switch
    unsigned scrambling_key_int = llvm::cryptoutils->scramble32(switchI->getNumCases(), scrambling_key);
  ...
  for (vector<BasicBlock *>::iterator b = origBB.begin(); b != origBB.end();
    ...
    if (i->getTerminator()->getNumSuccessors() == 1) {
      ...
      if (numCase == NULL) {
        unsigned scrambling_key_int = llvm::cryptoutils->scramble32(
    ...
    if (i->getTerminator()->getNumSuccessors() == 2) {
      ...
      if (numCaseFalse == NULL) {
        scrambling_key_int = llvm::cryptoutils->scramble32(
                                                                      switchI->getNumCases() - 1, scrambling_key);
